				      ...
				      int compress)
{
  ...
  if (!x3f_get_image(x3f, &image, &ilevels, NONE, 0,
		     fix_bad, denoise, apply_sgain, wb, 0, 0, 0.7, 20, 0, 8.0, 1.3, 0.01) ||  /* Disable tone curve, sharpening, micro-contrast for DNG (raw output) */
      image.channels != 3) {
  ...
  if (!x3f_get_preview(x3f, &image, &ilevels, SRGB,
		       apply_sgain, wb, 300, &preview, 1)) {  /* Enable tone curve for preview */

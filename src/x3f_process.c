...

  {
    ...
    for (row = 0; row < image->rows; row++) {
      for (col = 0; col < image->columns; col++) {
        ...
        /* Tunable color correction matrix - adjust these values by eye
         * Default: identity matrix (no change)
         * To reduce green: reduce output[1] or increase R/B relative to G
         * Format: row-major 3x3 matrix
         */
        {
          static const double color_corr[9] = {
            1.00, 0.0, 0.0,  /* R row: R, G, B coefficients */
            0.0, 0.78, 0.0,  /* G row: R, G, B coefficients - reduce green */
            0.0, 0.0, 1.05    /* B row: R, G, B coefficients - boost B */
          };
          double corrected[3];
          for (color = 0; color < 3; color++) {
            corrected[color] = 
              color_corr[color*3 + 0] * output[0] +
              color_corr[color*3 + 1] * output[1] +
              color_corr[color*3 + 2] * output[2];
          }
          for (color = 0; color < 3; color++) {
